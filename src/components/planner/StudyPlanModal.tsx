...
interface StudyPlanModalProps {
    visible: boolean
    onClose: () => void
    selectStudyPlan?: StudyPlanType | null
    onSuccess: () => void
    allPlan: StudyPlanType[]
}

export default function StudyPlanModal({ visible, onClose, selectStudyPlan, onSuccess, allPlan }: StudyPlanModalProps) {
    const isEditing = !!selectStudyPlan
    const [isSaving, setIsSaving] = useState(false)
    const [showPicker, setShowPicker] = useState<'date' | 'start' | 'end' | null>(null)

    const [formData, setFormData] = useState<Partial<StudyPlanType>>({
        title: '',
        date: '',
        start: 8.0,
        end: 9.0,
    ...
    useEffect(() => {
        if (visible) {
            setFormData({
                title: selectStudyPlan?.title || '',
                date: selectStudyPlan?.date || new Date().toLocaleDateString('th-TH'),
                start: selectStudyPlan?.start || 8.0,
                end: selectStudyPlan?.end || 9.0,
                description: selectStudyPlan?.description || '',
            })
        }
    }, [visible, selectStudyPlan])

    const handleChange = (field: keyof StudyPlanType, value: string | number) => {
    ...
    const handleSave = async () => {
        ...
        const isOverlap = isEditing ?
            ...
            : allPlan.find(p => {
                return p.date === formData.date && p.start < (formData.end ?? 9.0) && p.end > (formData.start ?? 8.0)
            })

        if (isOverlap) {
            Alert.alert('ข้อผิดพลาด', 'แผนการเรียนนี้มีเวลาซ้อนกับแผนการเรียนอื่น')
            return
        }
        ...
        try {
            ...
            Alert.alert('เกิดข้อผิดพลาด', 'ไม่สามารถบันทึกแผนการเรียนได้ กรุณาลองใหม่อีกครั้ง')
            setIsSaving(false)
        } finally {
            setIsSaving(false)
        }
                        ...
                        <View style={styles.modalContainer}>
                            <View style={styles.header}>
                                <Text style={styles.headerTitle}>
                                    {isEditing ? 'แก้ไขแผนการเรียน' : 'เพิ่มแผนการเรียนใหม่'}
                                </Text>
                            </View>

                            <View style={styles.formContent}>
                                <Text style={styles.label}>หัวข้อแผนการเรียน</Text>
                                <TextInput
                                    style={styles.input}
                                    value={formData.title}
                                    onChangeText={(text) => handleChange('title', text)}
                                    placeholder="เช่น ทบทวน Data Structures"
